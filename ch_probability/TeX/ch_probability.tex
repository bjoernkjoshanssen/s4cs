...
\subsection{Probability and the real world}
    ...
    What if 100 heads in a row occur when you toss a coin---and the same happens to your friend?
    And your friend's friend?
    This should happen rarely indeed, but at what point do you put your foot down and declare it just should not happen?
    If we do not adopt Cournot's principle, probability theory is in danger of losing touch with reality.
	...
	\Vref{dieProp} shows this convergence for 100,000 die rolls.
	...
	Occasionally the proportion will veer off from the probability and appear to defy the Law of Large Numbers, as $\hat{p}_n$ does many times in \Vref{dieProp}. However, these deviations become smaller as the number of rolls increases.
...
\subsection{Disjoint or mutually exclusive outcomes}
	...
	Can you modify $D$ in \Vref{disjointSets} to make $A$, $B$, $D$ pairwise disjoint?\footnote{Yes, let $D=\{3,5\}$, or more generally $D\subseteq\{3,5\}$.}
	\end{exercise}
	...
	\begin{example}{Probability with two coins}%Thanks to Christian
...
In the bar plots \Vref{diceSumDist}, the bar heights represent the probabilities of outcomes. If the outcomes are numerical and discrete, it is usually (visually) convenient to make a bar plot that resembles a histogram, as in the case of the sum of two dice.
...
\end{eqnarray}
Combining~\eqref{complementSumTo1} and~\eqref{complementDisjointEquation} yields a very useful relationship between the probability of an event and its complement.
...
Sometimes we wonder if one outcome provides useful information about another outcome. The question we are asking is, are the occurrences of the two events independent? We say that two events $A$ and $B$ are independent if they satisfy~\eqref{eqForIndependentEvents}.
...
\termsub{Tree diagrams}{tree diagram} are a tool to organize outcomes and probabilities around the structure of the data.  \Vref{smallpoxTreeDiagram} shows the general idea.
\begin{figure}%[ht]
\centering
...
To make this abstract, consider $\P(X=1)=0.0392$ to be ``inoculated? Yes, 0.0392'',
$X=0$ to be ``no, not inoculated'', $\P(Y=1\mid X=1)=0.9754$ is then ``lived, 0.9754'' and so on.}
...
Bayes' Theorem is just a generalization of what we have done using tree diagrams. The numerator identifies the probability of getting both $A_1$ and $B$. The denominator is the marginal probability of getting $B$.
This bottom component of the fraction appears long and complicated since we have to add up probabilities from
all the different ways to get $B$. We always completed this step when using tree diagrams.
However, we usually did it in a separate step, so it didn't seem as complex.
...
In particular, you should find it useful to know the expansions in \Cref{142857}.
\begin{table}
\centering
...
Perhaps we should take $p=1/2$ as our starting assumption (this is consistent with the idea of maximum entropy from \Cref{maxEnt}).
...
\]
However, if we consider $p$ to be a random variable, then any particular value will have probability 0,
and we should be considering the \term{probability density function} (see \Cref{contDist}) instead:
...
Now $\P(X=1\mid p=p_0)$ is supposed to be $p_0$
(even though $p=p_0$ is a probability-zero event, which leads to dividing by zero if we literally using it in conditional probability)
because $\P(X=1)=p=p_0$ if $p=p_0$. So we get
\[
...
\end{itemize}
While the frequentist stance avoids the tricky issue ``what should our initial pdf, after seeing 0 values, be?'', the Bayesian stance is well-suited to machine learning, whereby a machine can be continuously updating its beliefs about the world using the method of \eqref{learnBayes}.
...
It is also possible to compute the expected value of a continuous random variable (see \Cref{contDist}):
\[
\mu = \int xf(x)dx
\]
...
\begin{definition}
The \term{uniform distribution} on a finite set of $n$ elements, $\{x_1,\dots,x_n\}$, is given by $\P(X=x_i)=\frac1n$.
If some $x_i$ are identical (say $k$ many are equal to a particular $x_i$) this makes that $x_i$ have probability $k/n$.
\end{definition}
...
The following subsections roughly correspond to chapters in a popular calculus text such as Stewart's \emph{Calculus} or
Hass, Weir and Thomas' \emph{University Calculus}.
...
\]
When we check the 68-95-99.7 rule in \Cref{997} we shall use Taylor series:
\[
