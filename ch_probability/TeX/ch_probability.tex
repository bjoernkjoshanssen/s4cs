...
Probability can be illustrated by rolling a die many times. Let $\hat{p}_n$ be the proportion of outcomes $X_1,\dots,X_n$ that are \resp{1} after the first $n$ rolls:
\[
\hat p_n = \frac1n\sum_{i=1}^n X_i.
\]
As the number of rolls increases, $\hat{p}_n$ will converge to the probability of rolling a \resp{1}, $p = 1/6$. Figure~\ref{dieProp} shows this convergence for 100,000 die rolls. The tendency of $\hat{p}_n$ to stabilize around $p$ is described by the \term{Law of Large Numbers}. 
...
\]
is independent of the sign $\pm$. Here we are using the fact that the \term{covariance} $\E(XY)-\E(X)\E(Y)$ is zero for independent random variables. This can be verified by integration $\iint dx\,dy$ which is covered in multivariable calculus.
\index{random variable|)}
...
For a continuous random variable with pdf $f_X$, we can naturally define a \term{mode} to be any point $x$ with $f'_X(x)=0$ and $f''_X(x)\le 0$, i.e., a local maximum of $f_X$.
