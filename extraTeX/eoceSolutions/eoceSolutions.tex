...
\eocesol{
See \href{https://docs.google.com/spreadsheets/d/13Rvset_K3SSyru6MohnliE0Bx9wqInj4s0KHK-108kc/edit?usp=sharing}{this spreadsheet} to find out.
}
...
\]
where $x=y/(1-y)$ and $y=x/(x+1)$ and $1-y=1/(x+1)$,
\[
=\frac{\sqrt{y(1-y)}}{(y/(1-y)) B(1/2,1/2)} \cdot  \frac{1}{(1-y)^2}
\]
\[
=\frac1{B(1/2,1/2)}y^{-1/2}(1-y)^{-1/2}.
\]
This is a beta distribution pdf with parameters $1/2$ and $1/2$.
}

\eocesol{
$b_1=1.22$ and $b_0=-1.07$ (from the \href{https://docs.google.com/spreadsheets/d/1u4C2hsPl6mPIVMiYWUAHkn12u9-0NzlxzMSMyp8ONAM/edit?usp=sharing}{spreadsheet})
so for example for the point $(x_5,y_5)=(4,2.05)$,
we have $\epsilon_5 = 2.05 - 4=-1.95$ and $e_5 = 2.05 - (-1.07 + 1.22\cdot 4)=-1.76$.
}
\end{multicols}
...
\]
\[
= -(2+e^{\beta_0+\beta_1x}+e^{-(\beta_0+\beta_1x)})^{-2}
\]
\[\cdot (e^{\beta_0+\beta_1x}\beta_1 -\beta_1 e^{-(\beta_0+\beta_1x)})
\]
...
\]
\[
	\lim_{x\to-\infty}\frac1{1+e^{-x}} = \frac1{1+\lim_{x\to-\infty} e^{-x}}\]
\[ = \frac1{1+\lim_{x\to\infty} e^{x}} = [1/\infty] = 0.
\]
...
\[
f_X(x) = \frac1{6-2}\cdot 1_{[2,6]}(x),
\]
or in plain language, $f_X(x)=1/4$ on the interval $[2,6]$ and 0 elsewhere.
Then
\[
\mathbb P(3 \le X \le 5) = \int_{3}^{5} f_X(x)\,dx = \int_3^5 \frac14\,.dx = \frac{5-3}4 = \frac12.
\]
And
\[
\mathbb E(X) = \int_{-\infty}^{\infty} x f_X(x)\,dx = \frac14 \int_2^6 x\,dx = \frac14(6^2/2-2^2/2)=\frac{16}4=4.
\]
}

\eocesol{
The random variable \(T\) has density
\[
f_T(t) = 2 e^{-2 t}, \qquad t \ge 0,
\]
Thus, the cumulative distribution function
\[
F_T(t) = \int_0^{t} 2e^{-2s}\,ds = [-e^{-2s}]^{t}_0 = 1 - e^{-2t}.
\]
and
\[
\mathbb{P}(T > 1) = 1 - \mathbb P(T \le 1) = 1 - (1 - e^{-2}) = e^{-2}.
\]
Moreover,
\[
\mathbb{E}[T] = \int_0^\infty t \cdot 2 e^{-2t}\,dt.
\]
With $u=t$ and $v'=2e^{-2t}$ this is
\[
\int_0^{\infty} u(t)v'(t)\,dt = 
[u(t)v(t)]_0^{\infty} -
\int_0^{\infty} u'(t)v(t)\,dt =
\]
\[
0 - \int_0^{\infty} (-e^{-2t})\,dt
= - \left[\frac{e^{-2t}}{2}\right]_0^{\infty} = \frac12.
\]
}
...
We determine $k$ from
\[
1 = \int_{-\infty}^{\infty} f(x)\,dx = \int_0^2 kx^2\,dx = k (2^3/3)
\]
which yields $k=3/8$.
Then
\[
\mathbb{P}(1 \le X \le 2) = \int_1^2 f(x)\,dx = \frac38 \int_1^2 x^2\,dx = \frac38(2^3/3-1^3/3) = 7/8.
\]

The variance computation is tedious; we do not recommend assigning it on exams. %similar using $\mathrm{Var}(X)=\mathbb E((X-\mathbb EX)^2)=\int_{-\infty}^{\infty} (x-)$
}

\eocesol{
The unconditional density \(f_L(t)\) is
\[
f_L(t) = 0.7 (e^{-t}) + 0.3 (3e^{-3t}),\quad t\ge 0.
\]
Then
\[
\mathbb{P}(L > 2)= \int_2^{\infty} f_L(t)\,dt = ...
\]
Finally, given the event $A$ that a device has lasted 2 hours, the conditional probability that it is in the first mode ($M_1$) is
\[
\mathbb P(M_1 \mid A) = \frac{\mathbb P(M_1 \cap A)}{\mathbb P(A)} = \frac{\mathbb P(A\mid M_1) \mathbb P(M_1)}{\mathbb P(A)}
\]
\[
\frac{\mathbb P(A\mid M_1) \mathbb P(M_1)}{\mathbb P(A\mid M_1) \mathbb P(M_1) + \mathbb P(A\mid M_2) \mathbb P(M_2)}
\]
Here
\[
\mathbb P(A\mid M_1)= \int_A e^{-t}\,dt
\]
When $A$ is the event $L> 2$, this is
\[
\int_2^{\infty} e^{-t}\,dt = e^{-2}.
\]
And
\[
\mathbb P(A\mid M_2)= \int_A 3e^{-3t}\,dt
\]
When $A$ is the event $L> 2$, this is
\[
\int_2^{\infty} 3e^{-3t}\,dt = e^{-6}.
\]
So
\[
\mathbb P(M_1 \mid L>2) = 
\]
\[
\frac{\mathbb P(L>2\mid M_1) \mathbb P(M_1)}{\mathbb P(L>2\mid M_1) \mathbb P(M_1) + \mathbb P(L>2\mid M_2) \mathbb P(M_2)}
\]
\[
=\frac{e^{-2} 0.7}{e^{-2} 0.7 + e^{-6} 0.3}
\]
