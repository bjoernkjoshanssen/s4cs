...
\section{Deriving formulas for linear regression}
	...
	Let
	\[
		\ell(b_0,b_1) = \sum_{i=1}^n (y_i-(b_0+b_1x_i))^2
	\]
	
	Next one finds the \emph{Hessian determinant}
	\[
		\det H_f =
		\begin{vmatrix}
			\ell_{b_0b_0} & \ell_{b_0b_1}\\
			\ell_{b_1b_0} & \ell_{b_1b_1}\\
		\end{vmatrix}
		=
		\ell_{b_0b_0}\ell_{b_1b_1} - \ell_{b_0b_1}\ell_{b_1b_0}
	\]
	where $\ell_x$ means the derivative of $\ell$ with respect to $x$, and $\ell_{xy}$ means the derivative of $\ell_x$ with respect to $y$.
	...
	hence it is nonnegative. Moreover, $\ell_{b_0b_0}>0$. According to further Calculus III theory this means that $\ell$ indeed has a minimum, i.e., we have
	successfully minized the errors (loss $\ell$) in our linear regression.
	
	According to a linear-algebraic extension of this multivariable calculus theory, we need the matrix
	\[
	\begin{bmatrix}
		\ell_{b_0b_0} & \ell_{b_0b_1}\\
		\ell_{b_1b_0} & \ell_{b_1b_1}\\
	\end{bmatrix}
	\]
	to be of the form $L L^T$ where $T$ denotes the \emph{transpose} and
	$L = \begin{bmatrix}
			a & 0\\
			b & c\\
		\end{bmatrix}$ for some real numbers $a,b,c$.
	This gives
	$L L^T= \begin{bmatrix}
			a^2 & ab\\
			ab & b^2+c^2\\
		\end{bmatrix}$
	and we can take $a = 2n$, $b=\overline x$, and $c=\sqrt{\overline{x^2}-(\overline x)^2}$.


	Now let us compare our value of $b_1$ to that stated in \emph{OpenIntro Statistics}:
	\begin{equation}
	...
	\begin{equation}\label{shall-have}
		b_1=r s_y/s_x.
	\end{equation}
	In other words,
	\[
...
\[
\text{(True model)}\qquad
y_i = \beta_0 + \beta_1 x_i + \epsilon_i,
\]
where $\beta_0,\beta_1$ are the unknown population parameters and
$\epsilon_i$ are the (unobservable) true errors.

\[
\text{(Fitted model)}\qquad
\hat{y}_i = b_0 + b_1 x_i,
\]
and the residuals are defined by
\[
...
A prediction interval in simple linear regression provides a range in which we expect a future
observation $Y_0$ to fall for a given predictor value $x_0$. If the fitted regression line is
$\hat{y}(x) = \hat{\beta}_0 + \hat{\beta}_1 x$, then a $100(1-\alpha)\%$ prediction interval for $Y_0$
is given by
\[
\hat{y}(x_0) \;\pm\; t_{n-2,\,1-\alpha/2} \cdot s \sqrt{ \,1 + \frac{1}{n} +
\frac{(x_0 - \bar{x})^2}{\sum_{i=1}^n (x_i - \bar{x})^2} },
\]
where $s = \sqrt{\text{RSS}/(n-2)}$ is the residual standard error, $\bar{x}$ is the mean of the
observed predictor values, and $t_{n-2,\,1-\alpha/2}$ is a critical value from the $t$-distribution
with $n-2$ degrees of freedom. The square root term accounts both for uncertainty in estimating the
regression line and the natural variability of new observations around the line.

In Google Sheets, the needed quantities can be computed from the data using built-in functions.
The predicted value $\hat{y}(x_0)$ can be obtained using \texttt{FORECAST.LINEAR}, the residual
standard error $s$ using \texttt{STEYX}, the critical $t$-value using \texttt{T.INV.2T}, and the
remaining parts of the standard error term using standard formulas for means, counts, and sums.
Combining these values using the formula above allows one to construct the prediction interval
...
\begin{definition}
Let\footnote{See e.g. David E. Bock, Paul F. Velleman, and Richard D. De Veaux: \emph{Stats: Data and Models}, 4th edition.}
\begin{equation}\label{do-not-forget}
	\SE(b_1) = \frac{\sqrt{\sum (y_i-\hat y_i)^2/(n-2)}}{\sqrt{\sum (x_i-\overline x)^2}}.
\end{equation}
\end{definition}
Of course, we can define $\SE(b_1)$ however we want but the point is that with the definition above \eqref{do-not-forget},
\begin{theorem}
$(b_1 - \beta_1) / \SE(b_1)$ has a $t$ distribution with $n-2$ degrees of freedom.
\end{theorem}
...
\begin{example}{}
Imagine points $(2n,2n+1)$ and $(2n+1,2n)$. The slope $b_1$ is 1 in the limit.
As the number of points increases, the numerator $\sqrt{\sum (y_i-\hat y_i)^2/(n-2)}$ may converge but the denominator $\sqrt{\sum (x_i-\overline x)^2}$ increases.
...
\begin{proof}
	...
	\begin{eqnarray*}
		...
		-\sum y\overline y  &=& \sum \hat y^2- \sum \hat y\overline y   - \sum y\hat y \\
		 \overline y\sum \hat y   + \sum y\hat y &=& \overline y\sum y   +  \sum \hat y^2
	\end{eqnarray*}
	...
	\begin{eqnarray*}
		...
		\overline{xy} - \overline x\cdot\overline y &=& b_1(\overline{x^2} - \overline{x}^2)\\
	\end{eqnarray*}
	which is our formula for $b_1$ from Equation (\ref{b1}).
\end{proof}
...
\end{theorem}
\begin{proof}
	Substituting \eqref{shall-have}, we must show any of the following equivalent statements:
	\begin{eqnarray*}
	...
	\end{eqnarray*}
	The last line is exactly \Cref{ssethm}.
...
\end{theorem}
\begin{proof}
By \Cref{aid} and \Cref{ssethm}, we have
\[
...
The $t$ statistic $t=b_1/\SE(b_1)$ (corresponding to a null hypothesis $\beta_1=0$) is also given by
	\[
		t=r\sqrt{(n-2)/(1-r^2)}.
	\]
\end{theorem}
...
\begin{proof}%[Proof of Theorem \ref{https://www.andrews.edu/}]
	Using $\SE(b_1)^2=\frac{\sum(y-\hat y)^2/(n-2)}{(n-1)s_x^2}$ \eqref{do-not-forget}
	and $b_1=rs_y/s_x$ \eqref{shall-have},
	% \[
	% t^2 = \frac{b_1^2}{\SE(b_1)^2} = \frac{(rs_y/s_x)^2}{\frac{\sum(y-\hat y)^2/(n-2)}{(n-1)s_x^2}} =
	% \frac{(rs_y)^2}{\frac{\sum(y-\hat y)^2/(n-2)}{(n-1)}} =
	% \]
	\begin{eqnarray*}
	...
	\end{eqnarray*}
	Since both sides are positive, it is equivalent that
	\begin{eqnarray*}
...
\begin{lemma}
Let $a, b, u_1, u_2$ be positive real numbers and let $r$ be a real number.
...
{\small
In general the $F$ distribution is that of $\frac{U_1/d_1}{U_2/d_2}$ where the $U_i$ are independent and $\chi^2$ with $d_i$ df.
The $t$ dist with $d$ df is the distribution of $T=\frac{Z}{\sqrt{V/\nu}}$ where $Z$ is standard normal
and $V$, independent of $Z$, is $\chi^2(\nu)$.
Therefore $T^2$ has the $F(1,d)$ distribution,
by letting $U_1=Z$ and $U_2 = V$.

$r^2$ actually has a beta distribution with parameters $1/2$ and $(n-2)/2$.

The general fact is that if $X$ has a $F(d_1,d_2)$ distribution then
$\frac{d_1X}{d_1X+d_2}$ has a Beta$(d_1/2,d_2/2)$ distribution.
Indeed, for a number $0<x<1$,
$\frac{d_1X}{d_1X+d_2}\le x$ if and only if
$X \le  \frac{d_2x}{(1-x)d_1}$,
...
\qquad x > 0.
\]
Why $d_1/2$ and $d_2/2$ as Beta parameters?
The underlying reason is that if $Y=\frac{G_1}{G_1+G_2}$ where the $G_i$ are independent and Gamma$(a_i,1)$
then $Y$ is Beta$(a_1,a_2)$. Moreover, a $\chi^2(k)$ distribution is the same as a 
