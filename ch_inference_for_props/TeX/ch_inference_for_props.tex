...
Typically, we do not know the true proportion $p$, so we substitute some value to check conditions and to estimate the standard error. For confidence intervals, usually the sample proportion $\hat{p}$ is used to check the success-failure condition and compute the standard error. For hypothesis tests, typically the null value -- that is, the proportion claimed in the null hypothesis -- is used in place of $p$. Examples are presented for each of these cases in Sections~\ref{confIntForPropSection} and~\ref{htForPropSection}.
...
\subsection{Thinking deeper about standard errors}
	...
	\end{exercise}
	And indeed, it is clear that when $n=1$, $\hat p(1-\hat p) = 0$ for both outcomes, and $\E(0)=0$.

	On the other hand, when we do assume $p_1=p_2$, the best estimate of $p_1$ and $p_2$ is
	\begin{equation}\label{pool}
		\hat p = \frac{\hat p_1n_1+\hat p_2n_2}{n_1+n_2}
	\end{equation}
...
pooled proportion $\hat p$ from \eqref{pool}, and then the standard error
\[
...
From this being 0.01 (since the width of the 95\%, hence $1.96\approx 2$ standard deviations, confidence interval is 0.02) and also $\hat p_y-\hat p_x=0.04$ we can deduce what $\hat p_x$ and $\hat p_y$ are (Exercise \ref{zain_email}).

{\tiny One student in Fall 2018 instead considered $\hat p = \hat p_y-\hat p_x$ and then
the ``standard error'' $\sqrt{\hat p(1-\hat p)/n}$, but this is treating $\hat p$ as a proportion. Note that $\hat p<0$ is possible, so $\hat p$ is not quite the same kind of beast as $\hat p_y$ and $\hat p_x$. }
...
In the setting of confidence intervals for a difference of two proportions, the two sample proportions are used to verify the success-failure condition and also compute the standard error, just as was the case with a single proportion. \emph{We do not use a pooled sample proportion, since we are note assuming $p_1=p_2$.}
...
\emph{In a hypothesis test where the null hypothesis is that $p_1 - p_2 = 0.03$, say, the sample proportions $\hat p_1$ and $\hat p_2$ are used for the standard error calculation rather than a pooled proportion.} Actually one could imagine using something like
\[
...
If each year has a small $p$-value like this, we can be quite sure that curving occurs, and
...
Since grade points are at least 0.3 apart, the sum of grade points in a class must be an integer multiple of 0.1.
Thus, in a class of $10^k$ students, the mean is an integer multiple of $10^{-(k+1)}$.
