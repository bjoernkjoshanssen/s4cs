...
A \term{mode} is represented by a prominent peak in the distribution. To be mathematically precise, we could let the mode be the value with the most occurrences. But it is common to have \emph{few} or \emph{no} observations with the same value in a data set, which makes this other definition useless for many real data sets. See \Cref{probability} for a definition of mode in that case.
...
\end{align*}
We divide by $n-1$, rather than dividing by $n$. To see why, consult \Cref{probability}. The \term{sample standard deviation} is then $s=\sqrt{s^2}$.
...
This topic is treated extensively in \emph{OpenIntro Statistics}. We confine ourselves to some remarks.
