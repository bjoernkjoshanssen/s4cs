...
This means that for each subset $F$ of $\{x_1,x_2,x_3\}$, there is an automaton $M$ in the list below such that $M$ accepts the strings in $F$, but not the strings in $\{x_1,x_2,x_3\}\setminus F$.
Accepting means that there is a path through the automaton, starting at the state labeled ``start'', which consists of edges labeled according to the word $x_i$, and ends in the accepting state (the state having two circles).
	\begin{tikzpicture}[shorten >=1pt,node distance=1.5cm,on grid,auto]
